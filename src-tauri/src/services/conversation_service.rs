...
use crate::db::models::{
    GenerationOptions, Message, MessageStreamChunk, NewMessage, NewToolCall, ToolResult,
};
...
impl ConversationService {
    ...
    pub async fn send_message(
        ...
        let target_model = if let Some(model_id) = routing.selected_model_id.as_deref() {
            self.model_repo.get_by_id(model_id).await?
        } else {
            ...
            if let Some(path) = model.file_path.clone() {
                ...
                if should_load {
                    ...
                    self.runtime_orchestrator
                        .maybe_preload_model(&path, &profile)
                        .await;
                    let mode = self.hardware_service.get_performance_mode(None).await;
                    let _ = self.inference_service.load_model(&path, &profile, mode).await;
                }
            }
        }
